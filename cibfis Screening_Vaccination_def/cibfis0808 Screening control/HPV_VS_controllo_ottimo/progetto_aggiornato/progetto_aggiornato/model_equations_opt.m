...
function [Xdot] = model_equations_opt(t,X)
...
global lambda d d1 alfa1 delta1 delta2 delta3 delta4 cp cq cn cc ck ca ...
       ...
       theta1 theta2 theta3 theta4 theta5 Lvect Lvect1
...
%         R=0;
%     end
    M=V+S+H+E+P+I1+I2+I3+A+R; %popolazione totale
    alfa1 = (beta*alfa*cn*ck*(1-cc*ca)*(E+theta1*H+theta2*P+theta3*I1+theta4*I2+theta5*I3))/M;
    %% equations
   
    %L = Lvect(fix(tt)+1); % ingresso
    %u1 = Lvect(fix(t)+1);
    u1 = Lvect(fix(t)+1);
    u2 = Lvect1(fix(t)+1);

    Xdot(1) = u1*S+delta4*S-(d+delta1)*V;
    %Xdot(1) = delta2*S+delta4*S-(d+delta1)*V;
    Xdot(2) = lambda+delta1*V-(u1+delta4+alfa1+d)*S;
    Xdot(3) = alfa1*S+delta3*alfa1*R+sigma2*H-(alfa2+cp*u2*gamma1*sigma1+d)*E;
    ...
    Xdot(10)= cp*u2*gamma1*sigma1*E+gamma2*sigma2*H+gamma3*sigma3*P+gamma4*sigma4*I1+gamma5*sigma5*I2+gamma6*sigma6*I3+gamma7*A-(delta3*alfa1+d)*R;